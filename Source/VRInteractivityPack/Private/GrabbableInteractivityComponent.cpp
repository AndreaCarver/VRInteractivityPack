...
UGrabbableInteractivityComponent::UGrabbableInteractivityComponent() {
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> SurfaceShapePlaneAsset(
		TEXT("/Game/Geometry/G_SurfaceShape_Plane.G_SurfaceShape_Plane"));
	if (SurfaceShapePlaneAsset.Succeeded()) {
		SurfaceShapePlane = SurfaceShapePlaneAsset.Object;
	}
	static ConstructorHelpers::FObjectFinder<UStaticMesh> SurfaceShapeSphereLargeAsset(
		TEXT("/Game/Geometry/G_SurfaceShape_SphereLarge.G_SurfaceShape_SphereLarge"));
	if (SurfaceShapeSphereLargeAsset.Succeeded()) {
		SurfaceShapeSphereLarge = SurfaceShapeSphereLargeAsset.Object;
	}
...
void UGrabbableInteractivityComponent::PostEditChangeProperty(FPropertyChangedEvent & PropertyChangedEvent) {
	...
	if (SurfaceShape == ESurfaceShape::SS_Plane) {
		//ProtectedSurfaceShape = LoadObject<UStaticMesh>(NULL,
		//	TEXT("/Game/Geometry/G_SurfaceShape_Plane.G_SurfaceShape_Plane"), NULL, LOAD_None, NULL);
		SurfaceShapeVisualizer->SetStaticMesh(SurfaceShapePlane);
	} 
		...
		SurfaceShapeVisualizer->SetStaticMesh(SurfaceShapeSphereLarge);
	} 
